...
class AffmatCleaner(object):
	...
	def clean_distmat(self):
		...
		"""
		oldname = 'distmats/{0} Segment {1} Distmat.fasta'.format(self.handle, self.segment)
	...
	def save_affmat(self):
		...
		self.distmat.to_hdf('{0} Segment Affmats.h5'.format(self.handle), mode='a', key='segment{0}'.format(self.segment))
