...
class MaxEdgeFinder(object):
	...
	def get_isolate(self):
		self.isolate = self.G.nodes()[self.node_idx]
	...
	def get_pwis_for_sink(self):
		...
		# idx = list(self.root.full.axis1).index(self.isolate)
		# idx = [i for i, sink in enumerate(self.df.root.full.axis1) if sink == self.isolate]
		self.df = self.df.loc[self.isolate]

	def get_older_pwis(self):
		print('Getting older PWIs...')
		# idxs = [i for i, source in enumerate(self.df.root.full.axis0) if source in self.earlier_nodes]
		# self.older_pwis = np.array([self.row[i] for i in idxs])
		# self.older_pwis = np.nan_to_num(self.older_pwis)
		self.older_pwis = self.df.loc[self.earlier_nodes].fillna(0)
	...
	def get_earlier_nodes_of_highest_pwi(self):
		...
		if self.older_pwis.max() > 0:
			...
			self.best_older_nodes = bestmatch_pos.index
		else:
